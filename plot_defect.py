...
def func_plotdefects(ax, centroids, phi, color_str, type_str, scale):
    ...
    for ii in range(0, N_defects):
        ...
        ax.scatter(x0, y0, s=20, color=color_str)

        if np.char.equal(type_str, "positive") == 1:
            ax.plot((x0, x1), (y0, y1), color_str, linewidth=1.0)
        else:
            for jj in range(0, 3):
...
def nematic_plot(x, y, u, v, ax=None, density=1, linewidth=None, color=None,
                 cmap=None, norm=None, transform=None, zorder=None,
                 start_points=None, minlength=0.1, maxlength=10.0,
    ...
    *integration_direction* : ['forward', 'backward', 'both']
    ...
    # added alpha (saaransh)
    lc = mcollections.LineCollection(streamlines, alpha=1.0,
                                     transform=transform,
                                     **line_kw)
...
if __name__ == '__main__':
    ...
    path = '/media/saaransh/Storage_14TB/saaransh/data'
    folder = '250923_NBH_R10/000/KY04000AL09'
    parameter_file = ('{}/{}__parameters.json'.format(path, folder))

    assert os.path.isfile(parameter_file), 'cant find parameter file'
    with open(parameter_file) as jsonFile:
        P = json.load(jsonFile)
    
    L = P['X_SIZE']
    h = P['SPACE_STEP']
    n_x = n_y = int(L/h)
    n_b = membraneField(P).n_b

    x = np.linspace(0, L, n_x, endpoint=False)
    y = np.linspace(0, L, n_x, endpoint=False)
    xgrid, ygrid = np.meshgrid(y, x)

    # concatenate all files
    n = np.zeros([1, 2, n_x, n_y])
    X = np.zeros([1, 2, n_b])

    # loading bay
    for i in range(int(np.ceil((P['MAX_TIME']/P['TIME_STEP'])/P['batch']))):

        hfn = h5py.File('{}/{}_{}__nematic.h5'.format(path, folder, str(i+1)), 'r')
        n = np.concatenate((n, np.array(hfn.get('dataset'))), axis=0)

        hfx = h5py.File('{}/{}_{}__membrane.h5'.format(path, folder, str(i+1)), 'r')
        X = np.concatenate((X, np.array(hfx.get('dataset'))), axis=0)
    ...
    _, map_p, map_m = func_defectfind(qx, qy, filter_radius=5, switchsign=0)

    # search map and identify circular regions of positive and negative charge
    centroids_p = func_defectpos(map_p, areathresh=20)
    centroids_m = func_defectpos(map_m, areathresh=20)

    # get the oriengation of defects
    phi_p = func_defectorient(centroids_p, qx, qy,
        filter_radius=5, type_str="positive")
    phi_m = func_defectorient(centroids_m, qx, qy,
        filter_radius=5, type_str="negative")

    fig = plt.figure(1, dpi=200.0, figsize=[10, 12])
    ax = fig.add_subplot(111, box_aspect=1)
    ax.set_xlim(left=0, right=L-h)
    ax.set_ylim(bottom=0, top=L-h)
    ax.tick_params(left=False, bottom=False, labelleft=False, labelbottom=False)

    nematic_plot(x, y, qx.T, qy.T, density=4.0, linewidth=1, color='#000000')
    defect_scale = 1

    p_pos = centroids_p*h
    m_pos = centroids_m*h
    func_plotdefects(ax, p_pos, phi_p, '#ff4500', "positive", defect_scale)
    func_plotdefects(ax, m_pos, phi_m, '#3286aa', "negative", defect_scale)

    ax.scatter(np.mod(X[t,0,],L), np.mod(X[t,1,],L), s=121,
               marker='.', color='#000000', alpha=0.8)
